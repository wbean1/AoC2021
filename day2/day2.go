...
func RunPart1Rules(input []string) (int, int) {
	...
	for _, str := range input {
		...
		if strings.Contains(str, "forward") {
			horiz += number
		} else if strings.Contains(str, "down") {
			depth += number
		} else if strings.Contains(str, "up") {
			depth -= number
		} else {
...
func RunPart2Rules(input []string) (int, int) {
